...
#define IMX477_LINK_FREQ	600000000
...
#if 0 /* Original dual-PLL (RPi) register table — commented out */
/* Sensor mode registers for 2028x1520 @ 20fps, 2-lane MIPI (dual PLL, VCO=2100/1800) */
static const struct imx477_reg mode_2028x1520_regs_old[] = {
	{0x0136, 0x18},
	{0x0137, 0x00},
	{0x3c7e, 0x08},
	{0x3c7f, 0x02},
	...
	{0x0307, 0x5e},
	{0x0309, 0x0a},
	{0x030b, 0x02},  /* IOP_SYCK_DIV: divide by 2 (VCO=1800MHz) */
	{0x030d, 0x02},
	{0x030e, 0x00},  /* IOP PLL multiplier high byte: 150 (0x0096) */
	{0x030f, 0x96},  /* IOP PLL multiplier low: 150 (24/2*150=1800MHz VCO, /2=900MHz out) */
	{0x0310, 0x00},
...
#endif /* Original dual-PLL register table */

/*
 * Single-PLL register table from timeings.md
 * VCO = 24/4 × 200 = 1200 MHz
 * VTPXCK = 1200/2/5 = 120 MHz, pixel_rate = 120×4 = 480 Mpps
 * IOPSYCK = 1200/1 = 1200 Mbps/lane, link_freq = 600 MHz (DDR)
 * LLP = 4400, FLL = 5454, fps = 480M/(4400×5454) ≈ 20, H_blank = 172
 */
static const struct imx477_reg mode_2028x1520_regs[] = {
	/* Sensor init registers (unchanged from RPi IMX477 mode3) */
	{0x0136, 0x18},
	{0x0137, 0x00},
	{0x3c7e, 0x08},
	{0x3c7f, 0x02},
	...
	{0x938d, 0x41},
	{0x5078, 0x01},
	/* Data format and lane config */
	{0x0112, 0x0a},
	{0x0113, 0x0a},
	{0x0114, 0x01},  /* CSI_LANE_MODE: 2 lanes */
	/* Frame timing */
	{0x0342, 0x11},  /* LINE_LENGTH_PCK[15:8]: 4400 (0x1130) — H_blank=172 */
	{0x0343, 0x30},  /* LINE_LENGTH_PCK[7:0] */
	{0x0340, 0x15},  /* FRM_LENGTH_LINES[15:8]: 5454 (0x154E) → 20 fps */
	{0x0341, 0x4e},  /* FRM_LENGTH_LINES[7:0] */
	{0x0350, 0x00},
	{0x3210, 0x00},
	...
	{0x034f, 0xf0},
	/*
	 * PLL: Single PLL mode (timeings.md)
	 * VCO = 24/4 × 200 = 1200 MHz
	...
	{0x0309, 0x0a},  /* IOP_PXCK_DIV: 10 (RAW10) */
	...
	{0x3f26, 0x00},
	/* Exposure and gain */
	{0x0202, 0x15},  /* COARSE_INTEG_TIME: 5450 (0x154A) = FLL-4 */
	{0x0203, 0x4a},
	{0x0204, 0x00},
	{0x0205, 0x00},
	{0x020e, 0x01},
...
static const struct imx477_mode supported_modes[] = {
{
	...
	.vblank = 3934,        /* FLL - height = 5454 - 1520 */
	.vblank_min = 20,      /* FLL_min ≈ 1540 */
					   ...
					   size_t count, loff_t *ppos)
{
	...
	if (n == 2) {
		...
		dev_info(imx477->dev, "REG WRITE: 0x%04X = 0x%02X (readback: 0x%02X)\n",
			 addr, val, regval);
	} else {
		/* Read */
